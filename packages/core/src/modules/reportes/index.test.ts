...
describe("reporteDiario - consolidado", () => {
  function mockDbSimple(secuencia: any[]) {
    ...
    // Simula las 8 queries de reporteDiario:
    // 1. select().from(ventas).where().groupBy()
    // 2. select().from(pedidos).where().groupBy()
    // 3. select().from(gastos).where().groupBy()
    // 4. select().from(gastos).innerJoin(catGasto).where().groupBy()
    // 5. select().from(gastos).innerJoin(catGasto).where()  ← sin groupBy
    // 6. select().from(adelantos).where().groupBy()
    // 7. select().from(multas).where()  ← sin groupBy
    // 8. select().from(devolucionesAnticipo).where().groupBy()
    const fromFn = vi.fn()
      // Query 1: ventas
      .mockReturnValueOnce(whereChain())
      // Query 2: pedidos
      .mockReturnValueOnce(whereChain())
      // Query 3: gastos por origen
      .mockReturnValueOnce(whereChain())
      // Query 4: gastos por categoría (innerJoin)
      .mockReturnValueOnce(innerJoinChain())
      // Query 5: gastos detalle (innerJoin, sin groupBy)
      .mockReturnValueOnce({
        innerJoin: vi.fn().mockReturnValue({
      ...
      // Query 6: adelantos
      .mockReturnValueOnce(whereChain())
      // Query 7: multas (sin groupBy)
      .mockReturnValueOnce({
        where: vi.fn().mockImplementation(nextResult),
      })
      // Query 8: devoluciones anticipo
      .mockReturnValueOnce(whereChain());
